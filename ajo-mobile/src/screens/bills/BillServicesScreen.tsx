...
import { getBillServices } from "../../api/bills";
...
export function BillServicesScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    ...
    getBillServices()
      .then((services) => {
        if (mounted) setEnabled(new Set(services));
      })
      .catch(() => {
        // Leave tiles empty on failure; the member only sees what the admin
        // has configured once the fetch succeeds on next visit.
      })
      .finally(() => {
  ...
  const services = loading ? [] : ALL_SERVICES.filter((s) => enabled.has(s.key));
  ...
  return (
    <Screen>
      ...
      {!loading && services.length === 0 && (
        <Text style={styles.emptyNote}>Bill services are not available right now.</Text>
      )}
...
const styles = StyleSheet.create({
  ...
  emptyNote: { fontSize: typography.sizes.sm, color: colors.inkSoft, marginBottom: spacing.xl },
